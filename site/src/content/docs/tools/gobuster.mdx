...
description: "Discover hidden directories, files, DNS subdomains, and virtual hosts on web servers"
sidebar:
  order: 17
---
...
- **fuzz** -- basic fuzzing (for more advanced fuzzing, see [ffuf](/networking-tools/tools/ffuf/))
...
gobuster dns -d example.com -w wordlists/subdomains-top1million-5000.txt -t 10

# Show IP addresses for discovered subdomains
gobuster dns -d example.com -w wordlists/subdomains-top1million-5000.txt --show-ips -t 10

# Use custom DNS resolver to bypass caching
gobuster dns -d example.com -w wordlists/subdomains-top1million-5000.txt -r 8.8.8.8:53 -t 10
```
...
- For more flexible fuzzing (POST data, headers, parameter discovery), see [ffuf](/networking-tools/tools/ffuf/) which supports a FUZZ keyword that can be placed anywhere in a request.

## Related Tools

- [ffuf](/networking-tools/tools/ffuf/) -- flexible web fuzzer with FUZZ keyword for parameter, header, and POST data fuzzing
- [nikto](/networking-tools/tools/nikto/) -- web server vulnerability scanner (checks for known issues, not brute-force)
- [nmap](/networking-tools/tools/nmap/) -- port scanning and service detection (discover what ports are open before directory scanning)
